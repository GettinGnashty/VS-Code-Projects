public class Advent_day2_puzzle2 {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < myList.length; i++) {
            ...
            int dampener = 0;
            int compareToMe = myList[i][0]; //the first time the current array is looped thru, sets the compareToMe to current array's first number
            while(dampener < 2){
                for (int j = 1; j < myList[i].length; j++) {
                    ...
                    if (Math.abs(currentIndex - compareToMe) > 3 || compareToMe == currentIndex) {
                        if (dampener == 0){
                            dampener++;} 
                            else{unsafe++;
                                break;
