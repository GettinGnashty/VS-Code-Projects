...
public class Advent_day2_puzzle1 {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < myList.length; i++) {
            ...
            compareToMe = myList[i][0]; //the first time the current array is looped thru, sets the compareToMe to current array's first number
            
            for (int j = 1; j < myList[i].length; j++) {
                int currentIndex = myList[i][j];
                if (currentIndex - compareToMe > 0) { //checks if its increasing
                    isIncreasing = true; //is resetting itself back to false, needs to be fixed
                }
                if (currentIndex - compareToMe < 0) {//checks if its decreasing
                    isDecreasing = true;
                }
                if (isIncreasing == true && isDecreasing == true){ //if its both increasing and decreasing, is unsafe
                    unsafe++;
                    isIncreasing = isDecreasing = false;
                    break;
                }

                if (currentIndex - compareToMe > 3 || currentIndex - compareToMe < -3 || compareToMe == currentIndex) { //if the difference is more than 3 or zero, is unsafe
                    unsafe++;
                    break;
                } else {
                    compareToMe = myList[i][j];
                }
            }
        }
        System.out.println(myList.length - unsafe);
    }
}
